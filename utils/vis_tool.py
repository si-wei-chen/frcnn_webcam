...
def vis_bbox(img, bbox, label=None, score=None, ax=None):
    ...
    for i, bb in enumerate(bbox):
      if label is not None and label_names is not None:
        ...
        if label_names[lb] == 'person':
          xy = (bb[1], bb[0])
          height = bb[2] - bb[0]
          width = bb[3] - bb[1]
          #ax.add_patch(plot.Rectangle(
            #xy, width, height, fill=False, edgecolor='red', linewidth=2))
          cv2.rectangle(bbox_array, (bb[1], bb[2]), (bb[3], bb[0]), (255, 0, 0), 2)
          ...
          if len(caption) > 0:
            ...
            cv2.putText(bbox_array, ': '.join(caption),
                        (bb[1], bb[0]), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (255, 0, 0), 2)
